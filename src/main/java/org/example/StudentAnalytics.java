...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
        ...
        OptionalDouble averageAge = Arrays.stream(studentArray)
                .parallel()
                .filter(x -> x.checkIsCurrent() == true)
     ...
     * 65. A student has only failed the course if they have a failing grade
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
        ...
        long count = Arrays.stream(studentArray)
                    .parallel()
                    .filter(x -> ((x.checkIsCurrent() == false) && (x.getAge() > 20) && (x.getGrade() < 65)))
                    .count();
        return ((int) count);
    }
}
